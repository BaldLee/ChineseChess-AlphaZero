...
def expanding_data2(data):
    ...
    if data['result'] == 'red':
        value = 1
    elif data['result'] == 'black':
        value =0
    for move in data['move_list']:
        try: 
            policy = build_policy(move,flip=False)
        except Exception as e:
            logger.error(f"Expand data error {e}, move = {move}, state = {state}, data_id = {data['id']}")
            return convert_to_trainging_data(real_data, None)
...
def build_policy(action,flip):
    '''
        Translate an action to one-hot policy array
    '''
    policy = np.zeros(len(ActionLabelsRed))
    move_lookup = {
        move: i for move, i in zip(ActionLabelsRed, range(len(ActionLabelsRed)))
    }
#     print(move_lookup)
#     print(action)
#     print(move_lookup[action])
    policy[move_lookup[action]] = 1
    return policy
