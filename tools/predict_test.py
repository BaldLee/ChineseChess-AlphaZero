...
class tmp:
    ...
    def load(self, config_path, weight_path):
        if os.path.exists(config_path) and os.path.exists(weight_path):
            ...
            print(f"model files does not exist at {config_path} and {weight_path}")
            return False
    @staticmethod
...
config = tf.ConfigProto(allow_soft_placement=True,gpu_options=tf.GPUOptions(visible_device_list='0'))
